...
|---|---|---|
| 0. 프로젝트 스캐폴딩 | - | ✅ 완료 |
| 1. 도메인 모델링 & 인증 | 1~2주차 | 🔨 진행 중 (스키마 완료 / 엔티티·인증 미착수) |
| 2. 공연·좌석 조회 + 프론트 | 3~4주차 | ⬜ 대기 |
| 3. 가상 대기실 | 5~6주차 | ⬜ 대기 |
| 4. 좌석 선점 + 결제 + 예매 확정 | 7~8주차 | ⬜ 대기 |
| 5. Kafka 이벤트 연동 | 9~10주차 | ⬜ 대기 |
| 6. 부하 테스트 | 11주차 | ⬜ 대기 |
| 7. 마무리 | 12주차 | ⬜ 대기 |
...
- [ ] `global/common/BaseTimeEntity` — `created_at`/`updated_at` 공통 처리 (`@MappedSuperclass` + `@EntityListeners`)
- [ ] `global/config/JpaAuditingConfig` — `@EnableJpaAuditing`
- [ ] 도메인별 엔티티 10종 (담당 도메인별로 A/B 분담)
...
`global/` 패키지를 만들어뒀지만 계획에는 없던 작업입니다. 인증보다 **먼저** 잡아두면 이후 모든 API가 일관됩니다.
